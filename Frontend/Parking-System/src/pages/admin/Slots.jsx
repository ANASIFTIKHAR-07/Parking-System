      ...
      <div className='relative z-10 p-6 space-y-6'>
        ...
        <div className='flex items-center justify-between'>
          <div className="space-y-1">
            ...
            <p className='text-slate-600'>Manage individual parking slots and employee assignments</p>
          </div>
        </div>

        {/* Stats Cards */}
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
          <div className='backdrop-blur-xl bg-gradient-to-br from-blue-500/90 to-blue-600/90 rounded-2xl p-5 text-white shadow-lg border border-white/20'>
            <div className='flex items-center justify-between'>
              <div className="space-y-1">
                <p className='text-blue-100 text-xs font-medium uppercase tracking-wider'>Total Slots</p>
                <p className='text-2xl font-bold'>{slotsArray.length}</p>
              </div>
          ...
          <div className='backdrop-blur-xl bg-gradient-to-br from-green-500/90 to-emerald-600/90 rounded-2xl p-5 text-white shadow-lg border border-white/20'>
            <div className='flex items-center justify-between'>
              <div className="space-y-1">
                <p className='text-green-100 text-xs font-medium uppercase tracking-wider'>Available</p>
          ...
          <div className='backdrop-blur-xl bg-gradient-to-br from-purple-500/90 to-indigo-600/90 rounded-2xl p-5 text-white shadow-lg border border-white/20'>
            <div className='flex items-center justify-between'>
              <div className="space-y-1">
                <p className='text-purple-100 text-xs font-medium uppercase tracking-wider'>Assigned</p>
                <p className='text-2xl font-bold'>{assignedSlots}</p>
              </div>
              <div className='w-12 h-12 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center'>
                <svg className='w-6 h-6' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
          ...
          <div className='backdrop-blur-xl bg-gradient-to-br from-orange-500/90 to-red-600/90 rounded-2xl p-5 text-white shadow-lg border border-white/20'>
            <div className='flex items-center justify-between'>
              <div className="space-y-1">
                <p className='text-orange-100 text-xs font-medium uppercase tracking-wider'>Utilization</p>
                <p className='text-2xl font-bold'>{slotsArray.length > 0 ? Math.round((assignedSlots / slotsArray.length) * 100) : 0}%</p>
              </div>
              <div className='w-12 h-12 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center'>
                <svg className='w-6 h-6' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
        ...
        {/* Filters */}
        <div className='backdrop-blur-xl bg-white/40 rounded-2xl shadow-lg border border-white/20 p-5'>
          <h3 className='text-xl font-bold text-slate-800 mb-4'>Filter Slots</h3>
          <div className='grid grid-cols-1 gap-4 md:grid-cols-4'>
            <div>
              <label className='block text-sm font-medium text-slate-700 mb-1'>Floor</label>
              <select 
                className='w-full rounded-xl border-0 bg-white/60 backdrop-blur-sm px-3 py-2 text-slate-900 shadow-sm focus:ring-2 focus:ring-blue-500/30 transition-all duration-200' 
        ...
        {/* Bulk Create */}
        <div className='backdrop-blur-xl bg-white/40 rounded-2xl shadow-lg border border-white/20 p-5'>
          <h3 className='text-xl font-bold text-slate-800 mb-4'>Bulk Create Slots</h3>
          <form onSubmit={createBulk} className='space-y-4'>
            <div className='grid grid-cols-1 gap-4 md:grid-cols-4'>
              ...
              <div>
                <label className='block text-sm font-medium text-slate-700 mb-1'>End Number</label>
                <input
                  type='number'
                  className='w-full rounded-xl border-0 bg-white/60 backdrop-blur-sm px-3 py-2 text-slate-900 shadow-sm focus:ring-2 focus:ring-blue-500/30 transition-all duration-200'
                  value={bulk.end} 
        ...
        {/* Error Message */}
        {error && (
          <div className='backdrop-blur-xl bg-red-500/20 border border-red-400/30 rounded-2xl p-4 flex items-center space-x-3 shadow-lg'>
        ...
        {/* Slots Table */}
        {loading ? (
          <div className='backdrop-blur-xl bg-white/40 rounded-2xl p-8 flex items-center justify-center shadow-lg border border-white/20'>
            <div className='flex items-center space-x-3'>
              <div className='w-6 h-6 border-3 border-blue-600/30 border-t-blue-600 rounded-full animate-spin'></div>
              <span className='text-slate-700 font-medium'>Loading slots...</span>
            </div>
          </div>
        ) : (
          <div className='backdrop-blur-xl bg-white/40 rounded-2xl shadow-lg border border-white/20 overflow-hidden'>
            <div className='px-5 py-4 border-b border-white/20 bg-gradient-to-r from-slate-50/50 to-blue-50/50'>
            ...
            {slotsArray.length === 0 ? (
              <div className='text-center py-8'>
                ...
                <h3 className='text-lg font-bold text-slate-800 mb-1'>No parking slots found</h3>
                <p className='text-slate-600'>Create your first parking slots using bulk creation above.</p>
              </div>
            ) : (
                ...
                <table className='min-w-full'>
                  <thead>
                    <tr className="bg-gradient-to-r from-slate-50/50 to-blue-50/50">
                      <th className='px-4 py-3 text-left text-xs font-medium text-slate-700 uppercase tracking-wider'>Slot</th>
                      <th className='px-4 py-3 text-left text-xs font-medium text-slate-700 uppercase tracking-wider'>Floor</th>
                      <th className='px-4 py-3 text-left text-xs font-medium text-slate-700 uppercase tracking-wider'>Company</th>
                      <th className='px-4 py-3 text-left text-xs font-medium text-slate-700 uppercase tracking-wider'>Employee</th>
                      <th className='px-4 py-3 text-left text-xs font-medium text-slate-700 uppercase tracking-wider'>Vehicle</th>
                      <th className='px-4 py-3 text-left text-xs font-medium text-slate-700 uppercase tracking-wider'>RFID</th>
                  ...
                  <tbody className="divide-y divide-white/10">
                    {slotsArray.map((slot, index) => (
                      <tr 
                        key={slot._id} 
                        className={`transition-all duration-200 hover:bg-white/60 ${index % 2 === 0 ? 'bg-white/20' : 'bg-white/10'}`}
                      >
                        <td className='px-4 py-3'>
                          <div className='flex items-center space-x-3'>
                            <div className='w-10 h-10 bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl flex items-center justify-center shadow-md'>
                              <span className='text-white font-bold'>{slot.slotNumber}</span>
                            </div>
                            <div className="space-y-0.5">
                              <div className='font-medium text-slate-900'>Slot {slot.slotNumber}</div>
                              <div className='text-xs text-slate-500'>ID: {slot._id?.slice(-8)}</div>
                            </div>
                          </div>
                        </td>
                        <td className='px-4 py-3'>
                          <span className='font-medium text-slate-900'>Floor {slot.floor?.floorNumber || 'N/A'}</span>
                        </td>
                        <td className='px-4 py-3'>
                          <span className='font-medium text-slate-900'>{slot.company?.name || 'Not Assigned'}</span>
                        </td>
                        <td className='px-4 py-3'>
                          <span className='font-medium text-slate-900'>{slot.employee?.name || 'Not Assigned'}</span>
                        </td>
                        <td className='px-4 py-3'>
                          <span className='font-medium text-slate-900'>{slot.employee?.vehicleNumber || 'N/A'}</span>
                        </td>
                        <td className='px-4 py-3'>
                          <span className='font-medium text-slate-900'>{slot.employee?.rfid || 'N/A'}</span>
                        </td>
                        <td className='px-4 py-3'>
                          {slot.employee ? (
                            <span className='inline-flex items-center px-2.5 py-1 rounded-xl text-xs font-medium bg-gradient-to-r from-purple-500 to-indigo-600 text-white shadow-md'>
                              Assigned
                            </span>
                          ) : (
                            <span className='inline-flex items-center px-2.5 py-1 rounded-xl text-xs font-medium bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-md'>
                              Available
                            </span>
                          )}
                        </td>
          ...
          <div className='backdrop-blur-xl bg-white/40 rounded-2xl shadow-lg border border-white/20 p-5'>
            <div className='flex items-center justify-between mb-4'>
              ...
              <button
                ...
                className='w-8 h-8 rounded-xl bg-slate-200/50 hover:bg-slate-300/50 flex items-center justify-center transition-all duration-200 text-slate-600 hover:text-slate-800'
              >
            ...
            <div className='mb-4 p-3 bg-blue-50/50 rounded-xl border border-blue-200/50'>
